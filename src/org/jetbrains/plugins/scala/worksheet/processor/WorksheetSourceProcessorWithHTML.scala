...
object WorksheetSourceProcessorWithHTML {
  ...
  val refreshDuration = 100
  val javascriptRefresher =
    s"""
  ...
  def process(srcFile: ScalaFile, ifEditor: Option[Editor], iterNumber: Int): Either[(String, String), PsiErrorElement] = {
    ...
    val htmlFileSetup =
      s"""
        ...
        |val $htmlFileId = new File("/tmp/worksheetBuffer.html")
        |val $htmlOutputId = new PrintStream($htmlFileId)
        |$htmlOutputId.println(\"\"\"<html><head>$javascriptRefresher</head><body>\"\"\")
      """.stripMargin
    ...
    val root = if (!isForObject(srcFile)) srcFile
    else {
